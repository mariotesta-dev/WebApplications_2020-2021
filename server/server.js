...
const isLoggedIn = (req, res, next) => {
  ...
  return res.status(401).json({ error: 'Unauthorized (!isLoggedIn): please check user authentication.' });
}
...
app.get('/api/question/:id', async (req, res) => {
  formsDao.getQuestion(req.params.id)
    .then(forms => res.json(forms))
    .catch(() => res.status(500).end());
...
app.get('/api/answer/:id', async (req, res) => {
  formsDao.getAnswer(req.params.id)
    .then(forms => res.json(forms))
    .catch(() => res.status(500).end());
...
app.get('/api/closedanswers/:code', async (req, res) => {
  await formsDao.getClosedAnswers(req.params.code)
    .then(data => res.json(data))
    .catch(() => res.status(500).end());
});
...
app.get('/api/adminforms', isLoggedIn, async (req, res) => {
  await formsDao.getAllFormsById(req.user.id)
    .then(data => res.json(data))
    .catch(() => res.status(500).end());
});
...
app.get('/api/useranswers/:formId', isLoggedIn, async (req, res) => {
  await formsDao.getRepliesByFormId(req.user.id, req.params.formId)
    .then(data => res.json(data))
    .catch(() => res.status(500).end());
});
...
app.post('/api/useranswers', [
  ...
  try {
    ...
    res.status(503).json({ error: `Database error during the creation of form reply ${formId} by ${name}.` });
  }
});

// POST /api/forms
app.post('/api/forms', isLoggedIn, [
